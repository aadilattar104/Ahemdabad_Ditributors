// StackedQtyChart.jsx
// Stacked bar chart — Units Sold by SKU, X axis = month-year
// Props:
//   data      — [{ month, year, sku_name, total_qty }]
...
export default function StackedQtyChart({ data = [], skuList = [], isLoading = false }) {
  const pivotMap = {};
  data.forEach((r) => {
    ...
    pivotMap[key][r.sku_name] = (pivotMap[key][r.sku_name] || 0) + (r.total_qty || 0);
    pivotMap[key]._total = (pivotMap[key]._total || 0) + (r.total_qty || 0);
  });
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: "0 0 12px", fontSize: 13, color: "var(--color-text-tertiary)" }}>
        Monthly quantity breakdown per SKU
      </p>
